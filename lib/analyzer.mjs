...
async function getAIAnalysis(content, analysisType, modelName) {
  ...
  if (analysisType === 'organization') {
    prompt = `
      ...
      Format your response as a structured markdown document with the following sections:
  
      ## Overall Assessment
      [A brief overall assessment of the codebase structure]
  
      ## Suggestions
  
      ### 1. [Type of suggestion, e.g. Organization]
      **Description:** [Detailed description of the suggestion]
      **Impact:** [Potential impact of implementing this suggestion]
  
      ### 2. [Type of suggestion, e.g. Modularization]
      **Description:** [Detailed description of the suggestion]
      **Impact:** [Potential impact of implementing this suggestion]
  
      [Additional suggestions following the same format]
    `
  ...
  try {
    ...
    if (analysisType === 'organization') {
      // For organization analysis, return the markdown response directly
      return responseText.trim()
    } else if (analysisType === 'criticalFiles') {
      // For criticalFiles analysis, parse as JSON
      ...
      try {
        ...
        if (parsedResponse.criticalFiles) {
...
export async function analyzeCodebase(maxCriticalFiles, outputDir, modelName) {
  ...
  try {
    ...
    await fs.writeFile(organizationSuggestionsFile, organizationSuggestions)
    console.log(
      chalk.green(
        `AI Organization suggestions saved to ${path.join(
          outputDir,
          'organization-suggestions.md'
        )}`
      )
